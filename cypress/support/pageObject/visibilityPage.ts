class VisibilityPage {
    ...
    clickHideButton() {
        ...
        cy.get(`button[id="${this.zeroWidthBtn}"]`).should('have.css', 'width', '0px')
        cy.get(`button[id="${this.overlapBtn}"]`)
            .siblings('div').should('have.id', 'hidingLayer')
        cy.get(`button[id="${this.opacityBtn}"]`).should('not.be.visible')
        cy.get(`button[id="${this.visibilityHiddenBtn}"]`).should('not.be.visible')
        cy.get(`button[id="${this.displayNoneBtn}"]`).should('not.be.visible')
        cy.get(`button[id="${this.offscreenBtn}"]`).should('have.class', 'offscreen')
    }
}
